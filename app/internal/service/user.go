...
type UserPartialUpdate struct {
	...
	Email     string `json:"email" schema:"email"`
	Phone     string `json:"phone" schema:"phone"`
	Age       int64  `json:"age" schema:"age"`
	Gender    string `json:"gender" schema:"gender"`
	Hobby     string `json:"hobby" schema:"hobby"`
...
func (pu UserPartialUpdate) ToDomain() *domain.UserPartialData {
	...
	d.Set("city", pu.City)
	d.Set("gender", pu.Gender)
	d.Set("hobby", pu.Hobby)
	//TODO add age
...
func bcryptPassword(user *domain.User) error {
	...
	logrus.Debugf("pass = %s, PasswordHash = %s", user.Password, user.PasswordHash)
